...
빨간색 밑줄 부분은 가지고 있는 샘플 X의 복잡한 분포를 우리는 실제로 알고있지 못하기 때문에 $$$P(z|{x}_i)$$$를 계산할 수 없습니다. 다만, KLD는 항상 0 이상의 값을 갖는다는 특성이 있습니다.
...
KLD 부분의 식의 구현은 $$$q(z|x_{i})$$$는 주어진 샘플 $$$x_i$$$에 대해 Encoder를 통과해 얻어진 mu와 sigma를 따르는 정규분포가 될 것이며, $$$P(z)$$$는 $$$z~N(0, 1)$$$를 만족한다고 할때, 모두 정규분포이므로 간단하게 표현할 수 있습니다.
